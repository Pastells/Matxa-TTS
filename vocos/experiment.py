    ...
    def training_step(self, batch, batch_idx, optimizer_idx, **kwargs):
        ...
        if optimizer_idx == 1:
            ...
            if self.train_discriminator:
                ...
                loss_fm_mp = self.feat_matching_loss(fmap_r=fmap_rs_mp, fmap_g=fmap_gs_mp) / len(
                    fmap_rs_mp
                )
                loss_fm_mrd = self.feat_matching_loss(fmap_r=fmap_rs_mrd, fmap_g=fmap_gs_mrd) / len(
                    fmap_rs_mrd
                )
